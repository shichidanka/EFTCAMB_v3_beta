...
contains
    ...
    subroutine EFTCAMBOLAllocateModelSelection( self, Ini, eft_error )
        ...
        ! allocate OL_Omega:
        call allocate_parametrized_1D_function( self%OL_Omega, self%OLOmegamodel, 'OLOmega', '\Omega', eft_error )
        if ( eft_error == 1 ) return
        ! allocate OL_Gamma1:
        call allocate_parametrized_1D_function( self%OL_Gamma1, self%OLGamma1model, 'OLGamma1', '{\gamma_1}', eft_error )
        if ( eft_error == 1 ) return
        ! allocate OL_Gamma2:
        call allocate_parametrized_1D_function( self%OL_Gamma2, self%OLGamma2model, 'OLGamma2', '{\gamma_2}', eft_error )
        if ( eft_error == 1 ) return
        ! allocate OL_Gamma3:
        call allocate_parametrized_1D_function( self%OL_Gamma3, self%OLGamma3model, 'OLGamma3', '{\gamma_3}', eft_error )
        if ( eft_error == 1 ) return

        ! additional initialization of the function:
        call self%OL_Lambda%init_func_from_file ( Ini )
        call self%OL_Omega%init_func_from_file  ( Ini )
        call self%OL_Gamma1%init_func_from_file ( Ini )
        call self%OL_Gamma2%init_func_from_file ( Ini )
        call self%OL_Gamma3%init_func_from_file ( Ini )
    ...
    subroutine EFTCAMBOLInitModelParametersFromFile( self, Ini, eft_error )
        ...
        call self%OL_Omega%init_from_file  ( Ini )
        call self%OL_Gamma1%init_from_file ( Ini )
        call self%OL_Gamma2%init_from_file ( Ini )
        call self%OL_Gamma3%init_from_file ( Ini )
