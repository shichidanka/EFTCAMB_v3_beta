    ...
    subroutine EFTCAMBRPHAllocateModelSelection( self, Ini, eft_error )
        ...
        select case ( self%RPHwDE )
            ...
            case(5)
                ...
                call self%RPH_wDE%set_name( 'RPHw', 'w' )
            case default
                call allocate_parametrized_1D_function( self%RPH_wDE, self%RPHwDE, 'RPHw', 'w', eft_error )
                if ( eft_error == 1 ) return
        end select
        ! allocate RPH_PlanckMass:
        call allocate_parametrized_1D_function( self%RPH_PlanckMass, self%RPHmassPmodel, 'RPHmassP', '\tilde{M}', eft_error )
        if ( eft_error == 1 ) return
        ! allocate RPH_Kineticity:
        call allocate_parametrized_1D_function( self%RPH_Kineticity, self%RPHkineticitymodel, 'RPHkineticity', '{\alpha^{\rm K}}', eft_error )
        if ( eft_error == 1 ) return
        ! allocate RPH_Braiding:
        call allocate_parametrized_1D_function( self%RPH_Braiding, self%RPHbraidingmodel, 'RPHbraiding', '{\alpha^{\rm B}}', eft_error )
        if ( eft_error == 1 ) return
        ! allocate RPH_Tensor:
        call allocate_parametrized_1D_function( self%RPH_Tensor, self%RPHtensormodel, 'RPHtensor', '{\alpha^{\rm T}}', eft_error )
        if ( eft_error == 1 ) return
