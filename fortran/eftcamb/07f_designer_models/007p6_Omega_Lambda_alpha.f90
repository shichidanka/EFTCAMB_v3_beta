...
contains
    ...
    subroutine EFTCAMBOLAllocateModelSelection( self, Ini, eft_error )
        ...
        ! allocate OL_Lambda:
        call allocate_parametrized_1D_function( self%OL_Lambda, self%OLLambdamodel, 'OLLambda', '\tilde{\lambda}', eft_error )
        if ( eft_error == 1 ) return
        ! allocate OL_PlanckMass:
        call allocate_parametrized_1D_function( self%OL_PlanckMass, self%OLmassPmodel, 'OLmass', '\tilde{M}', eft_error )
        if ( eft_error == 1 ) return
        ! allocate OL_PlanckMass:
        call allocate_parametrized_1D_function( self%OL_Kineticity, self%OLkineticitymodel, 'OLkineticity', '{\alpha^{\rm K}}', eft_error )
        if ( eft_error == 1 ) return
        ! allocate OL_Braiding:
        call allocate_parametrized_1D_function( self%OL_Braiding, self%OLbraidingmodel, 'OLbraiding', '{\alpha^{\rm B}}', eft_error )
        if ( eft_error == 1 ) return
        ! allocate OL_Tensor:
        call allocate_parametrized_1D_function( self%OL_Tensor, self%OLtensormodel, 'OLtensor', '{\alpha^{\rm T}}', eft_error )
        if ( eft_error == 1 ) return

        ! additional initialization of the function:
        call self%OL_Lambda%init_func_from_file    ( Ini )
        call self%OL_PlanckMass%init_func_from_file( Ini )
        call self%OL_Kineticity%init_func_from_file( Ini )
        call self%OL_Braiding%init_func_from_file  ( Ini )
        call self%OL_Tensor%init_func_from_file    ( Ini )
    ...
    subroutine EFTCAMBOLInitModelParametersFromFile( self, Ini, eft_error )
        ...
        call self%OL_Lambda%init_from_file    ( Ini )
        call self%OL_PlanckMass%init_from_file( Ini )
        call self%OL_Kineticity%init_from_file( Ini )
        call self%OL_Braiding%init_from_file  ( Ini )
        call self%OL_Tensor%init_from_file    ( Ini )
