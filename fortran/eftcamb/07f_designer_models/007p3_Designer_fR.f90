...
contains
    ...
    subroutine EFTCAMBDesignerFRReadModelSelectionFromFile( self, Ini, eft_error )
    ...
    subroutine EFTCAMBDesignerFRAllocateModelSelection( self, Ini, eft_error )
        ...
        select case ( self%EFTwDE )
            ...
            case(5)
                ...
                call self%DesfRwDE%set_name( 'EFTw', 'w' )
            case default
                call allocate_parametrized_1D_function( self%DesfRwDE, self%EFTwDE, 'EFTw', 'w', eft_error )
    ...
    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that reads the parameters of the model from file.
    subroutine EFTCAMBDesignerFRInitModelParametersFromFile( self, Ini, eft_error )
        ...
        ! read B0:
        self%B0 = Ini%Read_Double( 'EFTB0', 0._dl )
    ...
    subroutine EFTCAMBDesignerFRInitBackground( self, params_cache, feedback_level, success, outroot )
        ...
        if ( DebugEFTCAMB ) then
            ...
            file_debug_2%unit = 34
            call file_debug_1%CreateFile( TRIM(outroot)//'debug_designer_fR_B.dat' )
    ...
    contains
        ...
        subroutine output( num_eq, ind, x, y, B )
            ...
            if ( DebugEFTCAMB ) then
                ...
                if ( is_open ) then
                    write (file_debug_2%unit,'(20E15.5)') x, Exp(x), Ricci, y(1), &
