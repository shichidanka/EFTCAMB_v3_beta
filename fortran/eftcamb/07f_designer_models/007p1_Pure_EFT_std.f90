...
contains
    ...
    subroutine EFTCAMBPureEFTstdAllocateModelSelection( self, Ini, eft_error )
        ...
        ! allocate Omega:
        call allocate_parametrized_1D_function( self%PureEFTOmega, self%PureEFTmodelOmega, 'EFTOmega', '\Omega', eft_error )
        ...
        select case ( self%EFTwDE )
            ...
            case(5)
                ...
                call self%PureEFTwDE%set_name( 'EFTw', 'w' )
            case default
                call allocate_parametrized_1D_function( self%PureEFTwDE, self%EFTwDE, 'EFTw', 'w', eft_error )
                if ( eft_error == 1 ) return
        end select
        ! allocate Gamma1:
        call allocate_parametrized_1D_function( self%PureEFTGamma1, self%PureEFTmodelGamma1, 'EFTGamma1', '{\gamma_1}', eft_error )
        if ( eft_error == 1 ) return
        ! allocate Gamma2:
        call allocate_parametrized_1D_function( self%PureEFTGamma2, self%PureEFTmodelGamma2, 'EFTGamma2', '{\gamma_2}', eft_error )
        if ( eft_error == 1 ) return
        ! allocate Gamma3:
        call allocate_parametrized_1D_function( self%PureEFTGamma3, self%PureEFTmodelGamma3, 'EFTGamma3', '{\gamma_3}', eft_error )
        if ( eft_error == 1 ) return

        ! allocate the other functions only if not Horndeski:
        if ( .not. self%PureEFTHorndeski ) then
            ! allocate Gamma4:
            call allocate_parametrized_1D_function( self%PureEFTGamma4, self%PureEFTmodelGamma4, 'EFTGamma4', '{\gamma_4}', eft_error )
            if ( eft_error == 1 ) return
            ! allocate Gamma5:
            call allocate_parametrized_1D_function( self%PureEFTGamma5, self%PureEFTmodelGamma5, 'EFTGamma5', '{\gamma_5}', eft_error )
            if ( eft_error == 1 ) return
            ! allocate Gamma6:
            call allocate_parametrized_1D_function( self%PureEFTGamma6, self%PureEFTmodelGamma6, 'EFTGamma6', '{\gamma_6}', eft_error )
            if ( eft_error == 1 ) return
        end if
