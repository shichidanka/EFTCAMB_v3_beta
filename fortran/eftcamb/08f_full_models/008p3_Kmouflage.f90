...
contains
    ...
    subroutine EFTCAMBKmouflageReadModelSelectionFromFile( self, Ini, eft_error )
    ...
    subroutine EFTCAMBKmouflageAllocateModelSelection( self, Ini, eft_error )
    ...
    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that reads the parameters of the model from file.
    subroutine EFTCAMBKmouflageInitModelParametersFromFile( self, Ini, eft_error )
        ...
        self%alphaU = Ini%Read_Double( 'alphaU', 0.2_dl   )
        self%gammaU = Ini%Read_Double( 'gammaU', 1._dl    )
        self%m      = Ini%Read_Double( 'm'     , 3._dl    )
        if ( self%Kmimic ) then
          self%eps2_0 = Ini%Read_Double( 'eps2_0' , 0.01_dl )
        else
          self%eps2_0 = Ini%Read_Double( 'eps2_0' , -0.01_dl )
        end if
        self%gammaA = Ini%Read_Double( 'gammaA', 0.2_dl   )
    ...
    subroutine EFTCAMBKmouflageInitBackground( self, params_cache, feedback_level, success, outroot )
        ...
        if ( DebugEFTCAMB ) then
            ...
            call file_debug_1%CreateFile( TRIM(outroot)//'debug_kmouflage_background_solution.dat' )
            call file_debug_2%CreateFile( TRIM(outroot)//'debug_kmouflage_background_eft.dat' )
        end if
    ...
    contains
        ...
        subroutine output( num_eq, ind, x, y )
            ...
            if ( DebugEFTCAMB ) then
                ...
                if ( is_open ) then
                    write (file_debug_1%unit,'(20E15.5)') a, adotoa, (adotoa- adotoaL)/adotoaL, (1+eps1)*Av**2 -1.,  OmegaPhi, p_phi/rho_phi, eps1, Av, K, y(2), chi
                end if
                inquire( unit=file_debug_2%unit, opened=is_open )
                if ( is_open ) then
                    write (file_debug_2%unit,'(20E15.5)') a, self%EFTOmega%y(ind),self%EFTOmega%yp(ind),self%EFTOmega%ypp(ind),self%EFTOmega%yppp(ind), self%EFTLambda%y(ind) , self%EFTLambda%yp(ind),self%EFTc%y(ind) ,self%EFTc%yp(ind)
                end if
            end if
