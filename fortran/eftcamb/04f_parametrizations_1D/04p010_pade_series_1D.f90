...
contains
    ...
    subroutine PadeSeriesParametrized1DInitFunctionFromFile( self, Ini )
        ...
        self%order_up   = Ini%Read_Int( TRIM(self%name)//'_Pade_order_N', 0 )
        self%order_down = Ini%Read_Int( TRIM(self%name)//'_Pade_order_D', 0 )
        ! allocate:
...
end module EFTCAMB_pade_series_1D
