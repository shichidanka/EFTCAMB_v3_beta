...
contains
    ...
    subroutine Spline1DInitFunctionFromFile( self, Ini )
        ...
        self%num_pixels   = Ini%Read_Int( TRIM(self%name)//'_Spline_Pixels', 0 )
        if ( self%num_pixels<2 ) then
            write(*,*) 'Spline requires ', TRIM(self%name)//'_Spline_Pixels', ' to be at least 2'
