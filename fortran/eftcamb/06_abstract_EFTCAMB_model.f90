...
module EFTCAMB_abstract_model
    ...
    contains
        ...
        procedure :: compute_QS_mu               => EFTCAMBModelComputeQSMu                               !< function that computes the quasi-static expression of mu.
        procedure :: compute_QS_Sigma            => EFTCAMBModelComputeQSSigma                            !< function that computes the quasi-static expression of Sigma.
    ...
    abstract interface
        ...
        subroutine EFTCAMBModelReadModelSelectionFromFile( self, Ini, eft_error )
            use IniObjects
            import EFTCAMB_model
            implicit none
            class(EFTCAMB_model)  :: self      !< the base class
        ...
        ! ---------------------------------------------------------------------------------------------
        !> Subroutine that reads the parameters of the model from file.
        subroutine EFTCAMBModelAllocateModelSelection( self, Ini, eft_error)
            use IniObjects
            import EFTCAMB_model
            implicit none
            class(EFTCAMB_model)  :: self      !< the base class
        ...
        ! ---------------------------------------------------------------------------------------------
        !> Subroutine that reads the parameters of the model from file.
        subroutine EFTCAMBModelInitModelParametersFromFile( self, Ini, eft_error )
            use IniObjects
            import EFTCAMB_model
            implicit none
            class(EFTCAMB_model)  :: self      !< the base class
...
contains
    ...
    function EFTCAMBModelComputeQSMu( self, a, k,  eft_par_cache, eft_cache )

      implicit none

      class(EFTCAMB_model)                          :: self          !< the base class
      real(dl), intent(in)                          :: a             !< the input scale factor.
      real(dl), intent(in)                          :: k             !< the input scale.
      type(TEFTCAMB_parameter_cache), intent(inout) :: eft_par_cache !< the EFTCAMB parameter cache that contains all the physical parameters.
      type(TEFTCAMB_timestep_cache ), intent(inout) :: eft_cache     !< the EFTCAMB timestep cache that contains all the physical values.

      real(dl) :: EFTCAMBModelComputeQSMu                            !< the return value of QS Mu
      ...
      call self%compute_secondorder_EFT_functions(a, eft_par_cache, eft_cache )

      EFTCAMBModelComputeQSMu =(2*(2*(1 + eft_cache%EFTOmegaV)*eft_cache%EFTc*(k**2 + 3*eft_cache%adotoa**2 &
            &- 3*eft_cache%Hdot) +a*(-36*eft_cache%adotoa**4*(1 + eft_cache%EFTOmegaV)*eft_cache%EFTOmegaP &
            &- 3*eft_cache%adotoa**3*eft_par_cache%h0_Mpc*(1 + eft_cache%EFTOmegaV)*(eft_cache%EFTGamma2V &
            &+ a*eft_cache%EFTGamma2P) +2*k**2*eft_cache%adotoa**2*(2*eft_cache%EFTGamma5V*eft_cache%EFTOmegaP &
            &+ a*eft_cache%EFTOmegaP**2 +2*(1 + eft_cache%EFTOmegaV + a*eft_cache%EFTOmegaP)*eft_cache%EFTGamma5P) &
            &+(1 + eft_cache%EFTOmegaV)*(4*k**2*eft_cache%EFTGamma5V*eft_cache%Hdot + 3*eft_par_cache%h0_Mpc*&
            &eft_cache%EFTGamma2V*eft_cache%Hdotdot) -eft_cache%adotoa*(1 + eft_cache%EFTOmegaV)*(eft_par_cache%h0_Mpc*&
            &(eft_cache%EFTGamma2V*(k**2 + 3*eft_cache%Hdot) + a*(k**2 - 3*eft_cache%Hdot)*eft_cache%EFTGamma2P) &
            &-12*eft_cache%EFTOmegaP*eft_cache%Hdotdot))))/(2*a*k**2*eft_cache%adotoa*(1 + eft_cache%EFTOmegaV &
            &+ 2*eft_cache%EFTGamma5V)*eft_cache%EFTOmegaP*(a*eft_par_cache%h0_Mpc*eft_cache%EFTGamma2V + eft_cache%adotoa*(&
            &-4*eft_cache%EFTGamma5V + a*eft_cache%EFTOmegaP)) -k**2*(1 + eft_cache%EFTOmegaV)*(a*eft_par_cache%h0_Mpc*&
            &eft_cache%EFTGamma2V + eft_cache%adotoa*(-4*eft_cache%EFTGamma5V + a*eft_cache%EFTOmegaP))**2 &
            &+2*k**2*eft_cache%adotoa*(1 + eft_cache%EFTOmegaV + 2*eft_cache%EFTGamma5V)*(a*eft_par_cache%h0_Mpc*&
            &eft_cache%EFTGamma2V + eft_cache%adotoa*(-4*eft_cache%EFTGamma5V + a*eft_cache%EFTOmegaP))*(2*eft_cache%EFTGamma5V &
            &+ a*(eft_cache%EFTOmegaP + 2*eft_cache%EFTGamma5P)) +2*k**2*(1 + eft_cache%EFTOmegaV + 2*eft_cache%EFTGamma5V)**2*(&
            &2*eft_cache%EFTc -a*(-4*eft_cache%EFTGamma5V*eft_cache%Hdot + eft_cache%adotoa*eft_par_cache%h0_Mpc*(&
            &eft_cache%EFTGamma2V + a*eft_cache%EFTGamma2P) -4*eft_cache%adotoa**2*eft_cache%EFTGamma5P)) &
            &+ 6*(1 + eft_cache%EFTOmegaV + 2*eft_cache%EFTGamma5V)**2*(2*eft_cache%EFTc*(eft_cache%adotoa**2 &
            &- eft_cache%Hdot) -a*eft_cache%adotoa*eft_par_cache%h0_Mpc*(eft_cache%adotoa**2 - eft_cache%Hdot)*(&
            &3*eft_cache%EFTGamma2V + a*eft_cache%EFTGamma2P) +4*a*eft_cache%adotoa*eft_cache%EFTOmegaP*(-3*eft_cache%adotoa**3 &
            &+ eft_cache%Hdotdot) +a*eft_par_cache%h0_Mpc*eft_cache%EFTGamma2V*(2*eft_cache%adotoa**3 &
            &- 4*eft_cache%adotoa*eft_cache%Hdot + eft_cache%Hdotdot)))

    end function EFTCAMBModelComputeQSMu

    ! ---------------------------------------------------------------------------------------------

    function EFTCAMBModelComputeQSSigma( self, a, k,  eft_par_cache, eft_cache )
      ...
      real(dl) :: EFTCAMBModelComputeQSSigma                         !< the return value of QS Sigma.
      ...
      EFTCAMBModelComputeQSSigma =(2*(2*(1 + eft_cache%EFTOmegaV)*eft_cache%EFTc*(k**2 + 3*eft_cache%adotoa**2 &
            &- 3*eft_cache%Hdot) +a*(-36*eft_cache%adotoa**4*(1 + eft_cache%EFTOmegaV)*eft_cache%EFTOmegaP &
            &- 3*eft_cache%adotoa**3*eft_par_cache%h0_Mpc*(1 + eft_cache%EFTOmegaV)*(eft_cache%EFTGamma2V &
            &+ a*eft_cache%EFTGamma2P) +2*k**2*eft_cache%adotoa**2*(2*eft_cache%EFTGamma5V*eft_cache%EFTOmegaP &
            &+ a*eft_cache%EFTOmegaP**2 +2*(1 + eft_cache%EFTOmegaV + a*eft_cache%EFTOmegaP)*eft_cache%EFTGamma5P) &
            &+(1 + eft_cache%EFTOmegaV)*(4*k**2*eft_cache%EFTGamma5V*eft_cache%Hdot + 3*eft_par_cache%h0_Mpc*eft_cache%EFTGamma2V&
            &*eft_cache%Hdotdot) -eft_cache%adotoa*(1 + eft_cache%EFTOmegaV)*(eft_par_cache%h0_Mpc*(eft_cache%EFTGamma2V*(k**2 &
            &+ 3*eft_cache%Hdot) + a*(k**2 - 3*eft_cache%Hdot)*eft_cache%EFTGamma2P) -12*eft_cache%EFTOmegaP*eft_cache%Hdotdot))))/(2*a*k**2*&
            &eft_cache%adotoa*(1 + eft_cache%EFTOmegaV + 2*eft_cache%EFTGamma5V)*eft_cache%EFTOmegaP*(a*eft_par_cache%h0_Mpc*&
            &eft_cache%EFTGamma2V + eft_cache%adotoa*(-4*eft_cache%EFTGamma5V + a*eft_cache%EFTOmegaP)) -k**2*(1 &
            &+ eft_cache%EFTOmegaV)*(a*eft_par_cache%h0_Mpc*eft_cache%EFTGamma2V + eft_cache%adotoa*(-4*eft_cache%EFTGamma5V &
            &+ a*eft_cache%EFTOmegaP))**2 +2*k**2*eft_cache%adotoa*(1 + eft_cache%EFTOmegaV + 2*eft_cache%EFTGamma5V)*(&
            &a*eft_par_cache%h0_Mpc*eft_cache%EFTGamma2V + eft_cache%adotoa*(-4*eft_cache%EFTGamma5V + a*eft_cache%EFTOmegaP))*&
            &(2*eft_cache%EFTGamma5V + a*(eft_cache%EFTOmegaP + 2*eft_cache%EFTGamma5P)) +2*k**2*(1 + eft_cache%EFTOmegaV &
            &+ 2*eft_cache%EFTGamma5V)**2*(2*eft_cache%EFTc -a*(-4*eft_cache%EFTGamma5V*eft_cache%Hdot &
            &+ eft_cache%adotoa*eft_par_cache%h0_Mpc*(eft_cache%EFTGamma2V + a*eft_cache%EFTGamma2P) &
            &-4*eft_cache%adotoa**2*eft_cache%EFTGamma5P)) + 6*(1 + eft_cache%EFTOmegaV + 2*eft_cache%EFTGamma5V)**2*(&
            &2*eft_cache%EFTc*(eft_cache%adotoa**2 - eft_cache%Hdot) -a*eft_cache%adotoa*eft_par_cache%h0_Mpc*(eft_cache%adotoa**2 &
            &- eft_cache%Hdot)*(3*eft_cache%EFTGamma2V + a*eft_cache%EFTGamma2P) +4*a*eft_cache%adotoa*eft_cache%EFTOmegaP*(&
            &-3*eft_cache%adotoa**3 + eft_cache%Hdotdot) +a*eft_par_cache%h0_Mpc*eft_cache%EFTGamma2V*(2*eft_cache%adotoa**3 &
            &- 4*eft_cache%adotoa*eft_cache%Hdot + eft_cache%Hdotdot)))

   end function EFTCAMBModelComputeQSSigma
