...
def run( verbose=True, overwrite=None, outdir=None ):
    ...
    forutils_dir  = os.path.abspath(os.path.join(os.path.join( camb_dir, os.pardir ),'forutils') )
    ...
    forutils_files_temp = []
    for f in forutils_files:
        forutils_files_temp.append( os.path.join( forutils_dir, f ) )
    forutils_files = forutils_files_temp

    l_camb     = create_file_objs(camb_files)
    l_eftcamb  = create_file_objs(eftcamb_files)
    l_forutils = create_file_objs(forutils_files)

    mod2fil_camb      = file_objs_to_mod_dict( file_objs=l_camb )
    mod2fil_eftcamb   = file_objs_to_mod_dict( file_objs=l_eftcamb )
    mod2fil_forutils  = file_objs_to_mod_dict( file_objs=l_forutils )
    ...
    for i in l_eftcamb:
        tmp=[]
        for j in i.uses:
            if j not in ignore_modules:
                if j.lower() not in mod2fil_forutils.keys():
                    try:
                        ...
                        try:
                            ...
                            print("\033[031mError\033[039m module \033[032m"+j+"\033[039m in file \033[032m"+i.file_name+"\033[039m not defined in any files. Skipping...")
        if len(tmp)>0:
...
def get_uses(infile=None, macros={}):
    ...
    for i in t:
        tmp=p(i)
        if tmp:
            uses.append( list(filter(None,tmp.groups()))[0].strip() )
